      ...
      <main className='max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-9 pb-12 sm:pb-16'>
        ...
        <section className='mt-8 sm:mt-10 lg:mt-12'>
          <div className='flex flex-wrap justify-center gap-3 sm:gap-4 md:gap-6 lg:gap-8'>
            {[
              ...
              { num: '7', label: 'UPCOMING' },
            ].map((stat) => (
              <div 
                key={stat.label} 
                className='bg-[#FEFCF6] border px-6 sm:px-10 md:px-12 lg:px-14 py-3 sm:py-4 rounded-2xl sm:rounded-3xl border-[#9D8365] flex flex-col items-center gap-1 min-w-[100px] sm:min-w-[120px]'
              >
                <h2 className='text-[22px] sm:text-[24px] md:text-[26px] font-semibold text-[#8B3A14]'>
                  {stat.num}
                </h2>
                <p className='text-[10px] sm:text-[11px] text-[#AA8565] font-medium tracking-wide'>
                  {stat.label}
                </p>
              </div>
            ))}
          ...
          <div className='flex flex-col lg:flex-row gap-8 lg:gap-10 xl:gap-12'>
            ...
            <div className='flex-1 flex flex-col gap-4 sm:gap-5 min-w-0'>
              ...
              <div className='grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-5'>
                {departments.map((dept, index) => (
                  <div 
                    key={index} 
                    className='group rounded-2xl sm:rounded-3xl border border-[#D4C5A8] bg-white px-4 py-4 text-center transition hover:shadow-[0_16px_50px_rgba(0,0,0,0.08)] hover:-translate-y-0.5'
                  >
                    <div className='mx-auto mb-3 inline-flex h-11 w-11 sm:h-14 sm:w-14 items-center justify-center rounded-2xl sm:rounded-3xl bg-[#F9F2E4] text-[22px] sm:text-[26px]'>
                      {dept.icon}
                    </div>
                    <h3 className='text-[14px] sm:text-[15px] font-semibold text-[#5C1F0A]'>
                      {dept.name}
                    </h3>
                    <p className='mt-1 sm:mt-2 text-[11px] text-[#8A7355]'>
                      {dept.members}
                    </p>
                  </div>
                ))}
            ...
            <div className='flex-1 flex flex-col gap-4 sm:gap-5 min-w-0'>
              <div className='flex items-center justify-between'>
                <h2 className='text-[16px] sm:text-[18px] font-semibold text-[#5C1F0A]'>
                  Upcoming Programs
                </h2>
                <Link 
                  to='/programs' 
                  className='text-[#8B3A14] text-[12px] sm:text-[13px] font-semibold hover:underline'
                >
                  See all →
                </Link>
              </div>
                  ...
                  >
                    <div className='flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3'>
                      <div className='flex-1 min-w-0'>
                        ...
                        <p className='text-[11px] sm:text-[12px] text-[#8A7355] mt-0.5'>
                          {program.ministry}
                        </p>
        ...
        <section className='mt-12 sm:mt-16 lg:mt-24'>
          <div className='rounded-2xl sm:rounded-[28px] border border-[#D4C5A8] bg-[#F8EBBA] px-4 sm:px-6 lg:px-8 py-5 sm:py-6 shadow-sm'>
            <p className='text-[10px] uppercase tracking-[0.25em] text-[#8A7355] mb-2 font-medium'>
              Scripture of the Day
            </p>
            <p className='text-[14px] sm:text-[16px] lg:text-[18px] italic text-[#5C1F0A] leading-7 sm:leading-8'>
              "The Lord is my shepherd; I shall not want. He makes me lie down in green pastures; He leads me beside still waters."
            </p>
            <p className='mt-3 sm:mt-4 text-[11px] sm:text-[12px] font-semibold text-[#8B3A14]'>
              Psalm 23:1–2
            </p>
          </div>
        </section>
